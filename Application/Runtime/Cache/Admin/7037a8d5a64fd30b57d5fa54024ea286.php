...
<div class="content">
    ...
    <div class="well">
    <a class="btn btn-primary" onclick="javascript:window.location.href='/index.php/Admin/Auth/add/id<?php echo ($v["id"]); ?>'">新增</a>
        <!-- table -->
        <table class="table table-bordered table-hover table-condensed">
            ...
            <tbody>
                <?php if(is_array($auth)): $i = 0; $__LIST__ = $auth;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr class="success">
                    <td><?php echo ($v["id"]); ?>}</td>
                    <td><?php echo (str_repeat('&emsp;', $v["level"]*2)); echo ($v["auth_name"]); ?></td>
                    <td><?php echo ($v["pid"]); ?></td>
                    <td><?php echo ($v["auth_c"]); ?></td>
                    <td><?php echo ($v["auth_a"]); ?></td>
                    <td><?php if( $v["is_nav"] ==1 ): ?>是<?php else: ?>否<?php endif; ?></td>
                    <td>
                        <a href=/index.php/Admin/Auth/edit/id/<?php echo ($v["id"]); ?>"> 编辑 </a>
                        <a href="javascript:void(0);" onclick="if(confirm('确认删除？')) location.href='/index.php/Admin/Auth/del/id/<?php echo ($v["id"]); ?>'"> 删除 </a>
                    </td>
